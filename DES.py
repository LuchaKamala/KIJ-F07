...
class des():
    ...
    def generateSubkey(self, c, d):
        ...
        cd = c+d
        for i in self.pc2:
           result.append(cd[i-1])
        return ''.join(result)

    def initiatePermutation(self, message):
        result = []
        for i in self.ip:
            result.append(message[i-1])
        return ''.join(result)

    def calculateE(self, right):
        result = []
        for i in self.e:
            result.append(right[i-1])
        return ''.join(result)

    def xorKER(self, subkey, ER):
        result = []
        for i in range(48):
    ...
    def sboxCalculation(self, data):
        ...
        sibi = []
        for x in range(8):
            i = int(per6[x][0] + per6[x][5], 2)
            j = int(per6[x][1:5], 2)
            if x == 0:
                sibi.append(self.s1[i][j])
            elif x == 1:
                sibi.append(self.s2[i][j])
            elif x == 2:
                sibi.append(self.s3[i][j])
            elif x == 3:
                sibi.append(self.s4[i][j])
            elif x == 4:
                sibi.append(self.s5[i][j])
            elif x == 5:
                sibi.append(self.s6[i][j])
            elif x == 6:
                sibi.append(self.s7[i][j])
            elif x == 7:
                sibi.append(self.s8[i][j])

        result = []
        for x in sibi:
            result.append("{0:04b}".format(x))
        return ''.join(result)

    def calculateP(self, data):
        result = []
        for x in self.p:
            result.append(data[x-1])
        return ''.join(result)

    def calculateF(self, right, subkey):
        er = self.calculateE(right)
        xor = self.xorKER(subkey, er)
        sb = self.sboxCalculation(xor)
        p = self.calculateP(sb)
        return p

    def inverseIP(self, data):
        result = []
        for x in self.iip:
            result.append(data[x-1])
        return ''.join(result)